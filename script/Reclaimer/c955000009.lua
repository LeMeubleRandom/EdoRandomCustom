...
function s.initial_effect(c)
	...
	c:RegisterEffect(e5)
    local hint_sub=Effect.CreateEffect(c)
    hint_sub:SetType(EFFECT_TYPE_SINGLE)
    hint_sub:SetProperty(EFFECT_FLAG_CLIENT_HINT)
    hint_sub:setDescription(aux.Stringid(id,1))
    local e_hint=Effect.CreateEffect(c)
    e_hint:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_GRANT)
    e_hint:SetRange(LOCATION_FZONE)
    e_hint:SetTargetRange(LOCATION_MZONE,0)
    e_hint:SetTarget(s.target)
    e_hint:SetLabelObject(hint_sub)
